...
template<typename T>
comp_result<T> gen_func(optional<case_attribs<T>> attribs)
{
  if (attribs)
  {
    std::this_thread::sleep_for(std::get<duration>(attribs.value()));
    return std::get<1>(attribs.value()) ? comp_result<T>(std::get<1>(attribs.value()).value()) : comp_result<T>(
        hard_fail());
  }
  else
  {
...
template<typename T>
struct op_group_trial_type_traits :
    op_group_type_traits<T>
{
  typedef case_attribs<T> attr_type;
  typedef struct
  {
    optional<attr_type> f_attrs, g_attrs;
  } case_type;
};

template<>
struct op_group_trial_traits<INT_SUM> :
    op_group_trial_type_traits<int>
{
...
template<>
struct op_group_trial_traits<DOUBLE_MULT> :
    op_group_trial_type_traits<double>
{
  INLINE_CONST static case_type cases[] = {
...
template<>
struct op_group_trial_traits<AND> :
    op_group_trial_type_traits<bool>
{
  INLINE_CONST static case_type cases[] = {
...
template<>
struct op_group_trial_traits<OR> :
    op_group_trial_type_traits<bool>
{
  INLINE_CONST static case_type cases[] = {
